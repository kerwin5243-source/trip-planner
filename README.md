...
- [x] **第四階段**：✅ 地點搜尋 (Nominatim) ✅ 天氣 (Open-Meteo) ✅ PDF 匯出 (瀏覽器列印，封面 + 每日行程 + 各模組) ✅ AI 行程生成 (自備 Claude key 瀏覽器直呼，或複製提示詞貼回匯入)
- [x] **打磨**：Toast 回饋、儀表板即時統計、行程時間軸 + 拖曳排序 + 跨日搬移、進場動畫、PNG icon
- [x] **測試**：Vitest 3041 筆（分帳守恆、日期/時間運算、快照往返、AI JSON 解析）— `npm test`
